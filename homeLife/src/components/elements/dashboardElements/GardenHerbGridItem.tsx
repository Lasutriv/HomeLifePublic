...
function GardenHerbGridItem(props: IGardenGridItemProps) {
    ...
    useEffect(() => {
        ...
        const checkIfFileExists = async () => {
            try {
                const response = await fetch(require("./../../../files/gardening/herb/" + props.image));
    ...
    return (
        <>
            <div className={"herb-grid-item " + (props.isSelected ? ("selected") : (""))} onClick={() => props.handleSelection(props.name)}>
                {/* Make sure require() works on image */}
                { isFileFound ? (
                    <img src={ require("./../../../files/gardening/herb/" + props.image) } alt={ props.imageAlt } />   
                ) : (
                    <></>
                )}
                <h3>{ props.name }</h3>
...
export default GardenHerbGridItem;